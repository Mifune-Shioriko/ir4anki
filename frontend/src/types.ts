...
/** Preview-POOL fields ride on every session/state response. When
 * preview_mode is false/absent the frontend renders the exact legacy UI.
 * The manual preview round is retired (2026-09-27): pool cards made on an
 * earlier day auto-release overnight, capped at release_daily_goal/day. */
export interface PreviewExtras {
  preview_mode?: boolean
  /** suspended new cards currently in the pool (release tomorrow, subject
   * to the daily cap) */
  preview_pool?: number | null
  /** pool cards made TODAY (auto-release tomorrow) */
  pending_release?: number | null
  /** daily auto-release cap (2026-09-27); null = disabled */
  release_daily_goal?: number | null
  /** remaining release slots today (cap − released today); null = disabled */
  release_budget_left?: number | null
  /** pacing-mode table + default (2026-09-14) — the start screens render
   * today's plan from these, never hardcoded numbers */
